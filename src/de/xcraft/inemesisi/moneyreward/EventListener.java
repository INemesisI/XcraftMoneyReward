...
public class EventListener extends XcraftEventListener {
	...
	public EventListener(XcraftPlugin plugin) {
		super(plugin);
		rManager = (RewardManager) plugin.pluginManager;
	...
	public void onPlayerJoin(PlayerJoinEvent event) {
		...
		if (lastplayed.get(Calendar.DAY_OF_MONTH) < current.get(Calendar.DAY_OF_MONTH)) {
			...
			if (rManager.reward(event.getPlayer().getName(), reward)
					&& cManager.isDailyRewardNotify()) {
				plugin.messenger.sendInfo(event.getPlayer(), Msg.REWARD_DAILY.toString(
	...
	public void onEntityDeath(EntityDeathEvent event) {
		if (cManager.isUseBlacklist()
				&& event.getEntity().hasMetadata("SpawnReason")
				&& cManager.getBlacklist()
						.contains(event.getEntity().getMetadata("SpawnReason").get(0).asString())) {
			return;
		}
		...
		if (this.isCamping(player)) {
			...
			if (rManager.reward(player.getName(), reward) && cManager.isMobRewardNotify()) {
				Replace[] replace = {Replace.PLAYER(player.getName()),
						Replace.REWARD(rManager.economy.format(reward)), Replace.MOB(event.getEntityType().getName().toLowerCase())};
				if (reward > 0) {
					plugin.messenger.sendInfo(player, Msg.REWARD_MOB.toString(replace), true);
				} else {
					plugin.messenger.sendInfo(player, Msg.PENALTY_MOB.toString(replace), true);
				}
			}
		}
	}
		...
		if (cManager.isUseBlacklist() && cManager.isremoveBlacklistedOnChunkunload()) {
			for (Entity e : event.getChunk().getEntities()) {
				if (e.hasMetadata("SpawnReason")
						&& cManager.getBlacklist().contains(
		...
		if (cManager.isUseCamping()) {
			...
			if ((((camp.getX() - r) <= curr.getX()) && ((camp.getX() + r) >= curr.getX())) //
				...
				if (rp.campkills == cManager.getCampingCap()) {
					plugin.messenger.sendInfo(player, Msg.ERR_CAMPING.toString(), true);
				}
				// check for penalty
				if (e.hasMetadata("SpawnReason") && e.getMetadata("SpawnReason").get(0).asString().equals("SPAWNER")
					...
					int penalty = cManager.getPenaltyAmount()
							* (rp.campkills - cManager.getPenaltyAfter());
					if (rManager.reward(player.getName(), penalty)) {
						if (rp.campkills == cManager.getPenaltyAfter() && inform) {
							plugin.messenger.sendInfo(player, Msg.PENALTY_CAMP.toString(), true);
						}
						if (rp.campkills % 10 == 0) {
							...
							cost *= 0.01;
							plugin.messenger.sendInfo(player, Msg.PENALTY_INFO.toString(
									Replace.PLAYER(player.getName()),
									Replace.REWARD(rManager.economy.format(cost))), true);
						}
					} else
						return false;
				}
